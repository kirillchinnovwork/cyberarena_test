...
func (s *S3Storage) PutBytesMeta(ctx context.Context, objectKey string, data []byte, contentType string, meta map[string]string) (string, int64, error) {
	opts := minio.PutObjectOptions{ContentType: contentType, UserMetadata: meta}
	_, err := s.client.PutObject(ctx, s.bucket, objectKey, bytes.NewReader(data), int64(len(data)), opts)
	if err != nil {
		return "", 0, err
	}
